...
There are many avenues for future work reguarding the applied use of iterative-quantile binning and IQNN models. First is developing a variable selection procedure that provides a reliable guidance for an analyst in selecting and ordering the feature variables for strongest IQNN model performance. A related area would be to optimize the tuning process to identify an efficient way to select the number of bins in each dimension without an exhaustive - and therefore computationally expensive - search. There are also many statistical procedures that use a $k$-nearest-neighbor where a rough but computationally efficient approximation may be useful. For example, \ktm{(cite author/paper)} use nearest neighborhoods to generate localized weights for ensemble classifiers; a process that could be augmented to use iterative quantile binned neighborhoods. 
