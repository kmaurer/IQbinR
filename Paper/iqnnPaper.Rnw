...
\noindent \textbf{Binning:} \vspace{-.3cm}
  ...
  \item Repeat the following for $j = 2,...,P$ :
    ...
    \begin{enumerate}[i]
    \setlength\itemsep{0cm}
    \item Relabel the $L_j=\displaystyle\prod_{d=1}^{j-1}\delta_d$ index sets defined in $(j-1)^{th}$ iteration as $\{B_1,B_2,...B_{L_j}\}$
    \item Subdivide each $B_\ell$ into  $\delta_j$ quantile bins over feature $X_j$ to create new index sets 
    $B_{\ell m} = \{i \hspace{.2cm} | \hspace{.2cm} i \in B_\ell$ and $b_{X_{j}}^q(x_{ij}) = m \} \hspace{.2cm} \forall \hspace{.2cm} \ell \in \{1,2,...L_j\}$
    \end{enumerate}
    \vspace{-.3cm}
  \item The resulting $L=\displaystyle\prod_{d=1}^{j}\delta_d$ index sets are relabeled $\{B_1,B_2,...B_L\}$ corresponding to:\\
    \vspace{-.3cm}
    \begin{enumerate}[i]
    \setlength\itemsep{0cm}
    \item iterative-quantile bin neighborhoods $\vec{\bold{x}}_\ell = \{\vec{x_i} \hspace{.2cm} | \hspace{.2cm}  i \in B_\ell \} $
    \item iterative-quantile bin hyper-rectangles \ktm{$(lower_{j,\ell} \hspace{.1cm} , \hspace{.1cm} upper_{j,\ell}] \in \mathbb{R}^p$}
    \end{enumerate}
    \vspace{-.3cm}
    
  \end{enumerate}
\caption{$p$-dimensional iterative-quantile binning algorithm}
...
In Subsection~\ref{unibin} we saw how quantile bin neighborhoods could be used to roughly approximate KNN regression and classification models. The adaptation to $p$-dimensional feature spaces using iterative-quantile binning is straightforward. If the iterative-quantile binning provides index sets $B_\ell = \{i \hspace{.15cm} | \hspace{.15cm} \vec{x}_i \text{ in bin }\ell \} \hspace{.15cm} \forall \hspace{.15cm} \ell = 1,...,L$, then the  use to construct neighborhood groups of training observations can be used to drive a predictive modeling
