...
While computational efficiency is important, a new nearest-neighbor method must demonstract comparable or favorable predictive accuracy to existing methods to be considered viable. In this section we compare test accuracy of the IQNN models against those of KNN and AKNN models using standard loss metrics; mean squared error (MSE) to assess regression models and misclassification error rates to assess classifiers. Ten real data sets with categorical responses and continuous features variables were selected to test classification accuracy; likewise, ten data sets with continuous response values and continuous features variables were selected to test regression accuracy. Each data set contains at least two continuous numeric feature variables and range in size from $n=150$ to $n=\ktm{12345}$. All data sets come from the University of California Irvin (\textit{UCI}) Machine Learning Repository and the Knowledge Extraction based on Evolutionary Learning (\textit{KEEL}) data repository (\citealt{Lichman2013}; \citealt{alcala2011keel}). See Table~\ref{tab:data_class} and Table~\ref{tab:data_reg} in Appendix~\ref{dataAppend} for a more detailed summary of each data set used for testing classification and regression, respectively.

Before fitting or testing the models, we cleaned and scaled each data set such that clear comparison of model can be made across all data sets. The units associated with each feature varible and the number of feature variables differed across all data sets, so data pre-processing was implemented to allow for more uniform comparisons of accuracy for IQNN, KNN and AKNN models. First, a center/scale transformation was used to standardize each feature variable to negate the impact of diverse meaurement units on the distance calculations necessary for the KNN and AKNN approaches. Secondly, variable selection was performed to narrow the feature space to only two features per data set to simplify comparison across all ten cases. This was done by fitting a random forest to part of the training data removing all but variables with the two highest importance scores; using the average reductions in MSE and Gini index as importance metrics for regression and classification case, respectively. While the variable selection method is somewhat arbitrary here, using high importance variables is done to allow stronger predictions under the constraint than might be had by selecting two features at random. Additionally, we elect to specify the iterative-quantile algorithm to bin the features in order of importance scores. 

% Next for each data set we tune each model using 100-fold cross-validation to select parameter values that minimizes the loss metric. For IQNN this entailed tuning across ($\delta_1$,$\delta_2$) pairs that control the number of bins in each dimesion. To avoid tuning exhaustively over all possible bin parameterization, we narrowed our tuning to use nearly-balanced bins ($\gamma=1$) that provide binned-neighborhood sizes closest to powers of two; thus tuning over pairs in the set $D=\{(\delta_1,\delta_2)  \hspace{.1cm} \mid \hspace{.1cm} \delta_1 = \delta_2 \pm 1 \hspace{.1cm} \text{and} \hspace{.1cm} \displaystyle\min_{\delta_1,\delta_2} | \frac{n}{\delta_1\delta_2} - 2^d|  \hspace{.1cm} \forall  \hspace{.1cm} d=1,2,\dots \}$.We then tuned the KNN and AKNN models over the corresponding set of neighborhood sizes $K = \{k | k=\text{round}(\frac{n}{\delta_1\delta_2}) \hspace{.1cm} \forall \hspace{.1cm} (\delta_1,\delta_2) \in D \}$. The loss values from the tuned models were then collected for 100 additional initializations of 100-fold cross-validation for each data set to account for variation that could arise from the random initialization of cross-validation. See Table~\ref{tab:tuning_class} and Table~\ref{tab:tuning_reg} in Appendix~\ref{dataAppend} for the tuned parameters and average 100-fold cross-validated accuracy values from each data set.

Next for each data set we tune each model using 10-fold cross-validation to select parameter values that minimizes the loss metric. For IQNN this entailed tuning across ($\delta_1$,$\delta_2$) pairs that control the number of bins in each dimesion. We narrowed our tuning to use all nearly-balanced bins ($\gamma=1$) that provide binned-neighborhood sizes that fell nearest to whole integers. This was done to avoid tuning exhaustively over all possible bin parameterizations where the structural differences differ minimally when using a large number of bins in each dimension. We then tuned the KNN and AKNN models over the corresponding set of neighborhood sizes, using the integer nearest to the average neighbors per bin in the IQNN tuning. The loss values from the tuned models were then collected for 100 additional initializations of 10-fold cross-validation for each data set to account for variation that could arise from the random initialization of cross-validation. See Table~\ref{tab:tuning_class} and Table~\ref{tab:tuning_reg} in Appendix~\ref{dataAppend} for the sets parameters considered in tuning, the tuned parameters selected and average 100-fold cross-validated accuracy values from each data set for classification and regression, respectively.

Figure~\ref{fig:categorical_accuracy} displays the accuracy results for the tuned regression and classification models for all considered data sets. These are visualized relative change in accuracy compared to the KNN model - fit with brute force - as the baseline model. For classification, we can compare the diffence from KNN in average cross-validated percent misclassified.  For regression, we compare the \ktm{average percent change from the KNN cross-validated MSE to remove differences response units across data sets.}

<<categorical_accuracy, echo=F,include=T,eval=T,fig.width=9, fig.height=4, out.width='.99\\linewidth', fig.pos='H',fig.align='center',tidy=F, cache=TRUE, fig.show='hold',fig.cap="3-nn examples times...",warning=FALSE>>=
load(file="../DataRepo/classification/classification_testing.Rdata")

results_class$type_pretty <- factor(results_class$type, labels=c("IQNN   ","KNN-brute   ","AKNN-cover   ", "AKNN-kd   ")) 
class_n <- sapply(1:length(tuned_param_list), function(x) tuned_param_list[[x]]$n)
results_class$name_size <- factor(paste(results_class$data_name, results_class$obs,sep="\n n="), 
                                  levels=paste(levels(results_class$data_name), class_n,sep="\n n="))

ggplot()+
  geom_line(aes(x=name_size,y=-diff_acc*100,color=type_pretty,group=type_pretty),size=1,data=results_class)+
...
In Figure~\ref{fig:categorical_accuracy} we see that for all regression models \ktm{there are ...}. For the classification models we see that IQNN has mixed performance with cases above and below the misclassification rates from KNN; whereas, the AKNN models tend to be at or above the misclassification rates from KNN. 
...
\hline
\end{tabular}
\caption{Data sets used in testing regression accuracies.}
\label{tab:data_reg}
\end{table}
\normalsize
...
\small
\centering
\begin{tabular}{|l|ll|llll|} \hline
Data & tuned-$k$ & tuned-$(\delta_1,\delta_2)$ & IQNN & KNN-brute & AKNN-cover &  AKNN-kd  \\
\hline  
  &  &  &  &  &  &  \\
\hline
\end{tabular}
\caption{Put tuned parameters and 100-fold cv classification error rates here}
...
\hline
\end{tabular}
\caption{Put tuned parameters and regression 100-fold cv MSEs here}
