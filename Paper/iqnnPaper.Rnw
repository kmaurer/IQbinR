...
Figure~\ref{fig:categorical_accuracy} displays the accuracy results for the tuned classification models for all considered data sets. This visualization compares the diffence in average cross-validated percent misclassified relative to brute-force KNN as a baseline.
...
To compare regression accuracy across the nearest-neighbor style models we assess the root mean squared error (RMSE) for the ten regression data sets in an analogous process to that used in subsection~\ref{classacc} above. We first clean, rescale and select the feature variables for the regression data sets. We additionally standardized the continous response value to eliminate the inconsistent units when comparing across data sets; thus the RMSE is in units of standard-deviation for all sets. The tuning process for each model again uses the "one standard-error" rule with 100 initializations of ten-fold cross-validation. See Table~\ref{tab:tuning_reg} in Appendix~\ref{dataAppend} for parameterizations considered in tuning, the parameters selected and average ten-fold cross-validated misclassification error rates from each regression model on each data set.
