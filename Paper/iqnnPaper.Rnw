...
Thus estimating the response using some appropriate summary statistic $S(.)$ over the response values over training observations from the iterative quantile bin containing the query point. We propose using a simple average of numeric responses for IQNN regression and a majority vote count of categorical responses for IQNN classification. The rightmost panel of Figure~\ref{fig:2d_iqbin_plots} visually demonstrates the predicted values from IQNN regression over the simulated bivariate feature space resulting from averaging the ten training data response values in each bin. 
...
Iterative-quantile binning was designed to provide a potential alternative to $k$-nearest neighborhoods within training data, but it is important to recognize how fundamental differences in parameterization and treatment of feature values may effect behavior within predictive modeling. KNN models have a much simpler paramerization, requiring only specification of an integer neighborhood size $k$, whereas IQNN models require a variable ordering and the number of bins in each dimension. The magnitude of values in each features dimension can hold major influence on $p$-dimensional distances between training and query points. Feature values are often standardized before constructing KNN models, in an attempt to allow more balanced contribution across feature dimensions when performing neighborhood queries. Interestingly, any \ktm{term for positive one-to-one?} scaling will impact the hyper-rectangular bin boundary values, but will \textit{not} changes the iterative-quantile neighborhoods because the unchanged ordering of observations in each dimension leads to the same set of training points in each quantile-based partition.
