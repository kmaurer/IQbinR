...
Figure \ktm{(add ref for figure here)} shows this process in the simple case where $\vec{x}_i \in \mathbb{R}^p$. For this example we have randomly generated 160 observations from a multivariate normal distribution, parameterized such that 
\begin{center}
...
\end{bmatrix}\right)$.
\end{center}
In the example we use iterative quantile binning with four bins per dimension to create iterative-quantile nearest-neighborhoods of size $k=10$.

\ktm{Put bivariate iqnn example here.}
