...
\subsection{Univariate Binned Neighborhoods}
\label{GenBinning}
...
It is clear that any new query point, $q \in \mathbb{R}$, can be allocated to a bin by comparing the value to the set of $J+1$ bin bounds; a computational process much faster than calculating the distances from the training points when $J$ is much smaller than $n$. For a training data set containing a single continuous numeric feature variable and new query point $q\in\mathbb{R}$, we define the \textit{bin neighbors} as the set of training points $\vec{x}_q = \{x_i \hspace{.2cm} | \hspace{.2cm} b_X(x_i) = b_X(q) \}$. It is clear that the distance between a query point and any of its bin neighbors is bounded
\begin{center}
$d(x_i,q) = (x_i - q) < (\beta_j - \beta_{j-1})  \hspace{.2cm} \forall \hspace{.2cm} x_i \in \vec{x}_q$,
\end{center}
however the number of bin neighbors vary depending on bin boundaries and the distribution of training points. Attempting to control $d(x_i,q)$ by setting regularly spaced intervals for any non-uniformly distributed training set would result in imbalanced bin counts; thus, a poor alternative to $k$-nearest neighborhoods. 

We define \textit{quantile-binning} through specifying that the $j^{th}$ bin interval, $(\beta_{j-1} , \beta_j]$, takes the form $(Q_X((j-1)/J),Q_X((j)/J)]$, where $Q_X(p)$ is the the $p^{th}$ empirical quantile using the inverse empirical distribution function \ktm{\citep{hyndman1996sample}}. The resulting partitions will have interval widths that vary base on distribution, but will contain an equal or nearly-equal numbers of training points, $x_i$. 

we can then use the quantile bin neighborhoods to roughly approximate KNN classification or regression modeling by using the quantile bin neighbors' response values $\vec{y}_q = \{y_i \hspace{.2cm} | \hspace{.2cm} x_i \in \vec{x}_q\}$. The query point is given an estimated response, $\hat{y_q} = S(\vec{y}_q)$, for some summary statistic $S(.)$ that is appropriate for response type. Note that the estimated responses for each bin may be pre-calculated and stored for quick retrieval in situations with a large number of query points; unlike KNN models where the estimated response needs to be calculated for each unique query point. 
