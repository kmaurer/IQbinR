...
Next for each data set we tuned the parameterization for each classifier using twenty initializations of ten-fold cross-validation and make the final model selection using the ``one-standard error" heuristic rule; selecting the most parsimonious parameterization that has an average misclassification rate within one standard error of the lowest observed rate \citep{friedman2001elements}. For IQNN this entailed tuning across ($\delta_1$,$\delta_2$) pairs that control the number of bins in each dimesion. We narrowed our tuning to select from only nearly-balanced bins ($\gamma=1$) that provide average binned-neighborhood sizes that fell nearest to whole integers. This was done to avoid tuning exhaustively over all possible bin parameterizations. We then tuned the KNN and AKNN models over the corresponding set of neighborhood sizes, $k$, to the integers nearest to the average binned-neighborhood sizes from the IQNN tuning. The loss values from the tuned models were then collected for 100 additional initializations of ten-fold cross-validation for each data set. See Table~\ref{tab:tuning_class} in Appendix~\ref{dataAppend} for parameterizations considered in tuning, the parameters selected and average ten-fold cross-validated misclassification error rates from each model on each data set.
 ...
 <<categorical_accuracy, echo=F,include=T,eval=T,fig.width=9, fig.height=3.5, out.width='.99\\linewidth', fig.pos='H',fig.align='center',tidy=F, cache=TRUE, fig.show='hold',fig.cap="3-nn examples times...",warning=FALSE>>=
