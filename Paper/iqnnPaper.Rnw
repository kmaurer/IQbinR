...
We define \textit{quantile binning} through specifying that the $j^{th}$ bin interval, $(\beta_{j-1} , \beta_j]$, takes the form $(Q_X(\frac{j-1}{J})\hspace{.2cm},\hspace{.2cm}Q_X(\frac{j}{J})]$, where $Q_X(p)$ is the the $p^{th}$ empirical quantile using the inverse empirical distribution function \ktm{\citep{hyndman1996sample}}. The quantile binning function $b_X^q(.) : x_i \rightarrow x^\ast_j$ defined as 
%
\begin{eqnarray}\label{rectbin}
...
\end{center}
and iterative quantile binning with four bins per dimension us used to create iterative-quantile nearest-neighborhoods of size $k=10$. The left and center panels demonstrates the two stages of quantile-based partitioning necessary for this bivariate feature space. 
...
This iterative quantile process can be generalized for binning a set of $p$-dimensional points $[x_{i1},x_{i2},...,x_{ip}]^t = \vec{x}_i \in \mathbb{R}^p \hspace{.2cm} \forall \hspace{.2cm} i \in \{1,...,n\}$ through the following algorithm:\\
...
\noindent \textbf{Binning:} \vspace{-.3cm}
  ...
  $\{B_1,B_2,...B_{\delta_1}\}$ such that $B_\ell = \{i \hspace{.2cm} | \hspace{.2cm} b_{X_1}^q(x_{i1}) = \ell \} \hspace{.2cm} \forall \hspace{.2cm} \ell \in \{1,2,...\delta_1\}$
  \item Repeat the following for $j = 2,...,P$ \{ 
  \begin{enumerate}[i]
  \setlength\itemsep{0cm}
  \item Relabel the $L=\displaystyle\prod_{d=1}^{j-1}\delta_d$ index sets defined in $(j-1)^{th}$ iteration as $\{B_1,B_2,...B_L\}$
  \item Subdivide each $B_\ell$ into  $\delta_j$ quantile bins over feature $X_j$ to create new index sets 
  $B_{\ell m} = \{i \hspace{.2cm} | \hspace{.2cm} i \in B_\ell$ and $b_{X_{j}}^q(x_{ij}) = m \} \hspace{.2cm} \forall \hspace{.2cm} \ell \in \{1,2,...L\}$
  \end{enumerate}
  \}
  \item Assign final labels $\{B_1,B_2,...B_L\}$ where $L=\displaystyle\prod_{d=1}^{j}\delta_d$ for index sets from the $p$ iteration
  \end{enumerate}
\vspace{.3cm}
\label{iqalgorithm}
\caption{Iterative Quantile Binning Algorithm}
\end{algorithm}

The binning structure and $p$-dimensional nested partitions of the feature space in $\mathbb{R}^p$ are stored as an R-tree structured list of depth $p$. The interval bounds for feature $X_{j-1}$ are act as a parent node branching down to the $\delta_j$ intervals for feature $X_j$. It is then simple to identify the iterative quantile bin containing a new query point $[q_1,...,q_p]^t = \vec{q} \in \mathbb{R}^p$ by finding the interval at the $j^{th}$ level of the R-tree containing $q_j$ then moving down the branches for each dimension. A benefit of this structure is that \ktm{finish thought here...}
