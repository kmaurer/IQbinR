...
\noindent%
{\it Keywords:} Non-parametric, Predictive Modeling, Partitioning, KNN, Binning
\vfill
...
\noindent \textbf{Binning:} \vspace{-.3cm}
  ...
  \item Repeat the following for $j = 2,...,p$ :
    ...
    to combine parent and child indices of sets into unique indeces
    \end{enumerate}
    \vspace{-.3cm}
\end{enumerate}
...
In Subsection~\ref{unibin} we used univariate quantile bin neighborhoods to roughly approximate KNN regression and classification models. An extension to $p$-dimensional feature spaces will use training observations within iterative quantile bins to predict responses for test observations. We propose \textit{iterative quantile nearest-neighbor} (IQNN) models for regression and classification where the predicted response for query point, $\vec{q}$, is defined as
...
An iterative quantile nearest-neighbor model roughly mimics the distance-based neighborhoods of a $k$-nearest-neighbor model using iterative quantile bin neighborhoods over the feature space of the training data. To assess the viability of these IQNN models we need to evaluate the computational costs for constructing the binning structure using training observations and for making predictions for test observations. We then compare IQNN model performance to KNN, cover-tree AKNN, and kd-tree AKNN using several real data sets. We begin by investigating the computation times using theoretical and empirical approaches in Subsection~\ref{timing}. We then empirically assess the predictive performance of IQNN classification and regression models in Subsection~\ref{accuracy}. 
...
Iterative quantile nearest-neighbor models requires computational investment for the construction of the R-tree of univariate quantile intervals over each of the $p$-dimensions.
%used for binning future observations with the iterative quantile binning function, $b_{\mathbf{X}}^{iq}(q)$. 
The dominating computational cost for creating the iterative quantile bins is from generating the $\delta_j$ quantiles within each of the $\prod_{d=1}^{j}\delta_d$ data partitions at each iterative step for $j=1,...,p$. Creating empirical quantile for n observations can be conducted in $O(nlog(n))$ time using a comparison-based quick-sort algorithm to order the values. %Interestingly, as the number of partitions increases with each iteration, the number of training values per bin are dropping.
...
The storage size of the resulting bin structure will increase as the number of partitions increase, because the $j^{th}$ level of the R-tree requires $O(L_j)$ space; a total space of size $O(\sum_j^p L_j)$. Note that the computational time required decreases and the computation storage increases as the number of partitions, $\delta_j$, at each iteration increases. Thus partitioning to a larger number of smaller bins requires less time but more storage space.
...
\hline 
  iris & \html{http://archive.ics.uci.edu/ml/machine-learning-databases/iris} \\ 
  wpbc & \html{http://archive.ics.uci.edu/ml/machine-learning-databases/breast-cancer-wisconsin} \\ 
  pima & \html{http://archive.ics.uci.edu/ml/machine-learning-databases/pima-indians-diabetes} \\
  yeast & \html{http://archive.ics.uci.edu/ml/machine-learning-databases/yeast} \\ 
  abalone & \html{http://archive.ics.uci.edu/ml/machine-learning-databases/abalone} \\ 
  waveform & \html{http://archive.ics.uci.edu/ml/machine-learning-databases/waveform} \\ 
  optdigits & \html{http://sci2s.ugr.es/keel/dataset.php?cod=199} \\ 
  satimage & \html{http://sci2s.ugr.es/keel/dataset.php?cod=71} \\ 
  marketing & \html{http://sci2s.ugr.es/keel/dataset.php?cod=163} \\
  seizure & \html{http://archive.ics.uci.edu/ml/machine-learning-databases/00388} \\ 
\hline 
 & \\
\textbf{Regression} & \\ 
\hline 
...
\hline 
  wpbc & \html{http://archive.ics.uci.edu/ml/machine-learning-databases/breast-cancer-wisconsin} \\ 
  wankara &  \html{http://sci2s.ugr.es/keel/dataset.php?cod=41} \\ 
  laser &  \html{http://sci2s.ugr.es/keel/dataset.php?cod=47} \\ 
  treasury &  \html{http://sci2s.ugr.es/keel/dataset.php?cod=42} \\
  quake & \html{http://sci2s.ugr.es/keel/dataset.php?cod=75} \\ 
  skillcraft &  \html{http://archive.ics.uci.edu/ml/machine-learning-databases/00272} \\
  anacalt & \html{http://sci2s.ugr.es/keel/dataset.php?cod=159} \\
  puma & \html{http://sci2s.ugr.es/keel/dataset.php?cod=1291} \\
  air quality & \html{http://archive.ics.uci.edu/ml/machine-learning-databases/00360} \\
  ccpp & \html{http://archive.ics.uci.edu/ml/machine-learning-databases/00294} \\
   \hline
\end{tabular}
