...
To empirically demonstrate the time required for binning and prediction we use simulated training and test data sets, such that we are able to control number of observations and features. We generate data of sizes ...
