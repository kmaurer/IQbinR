...
combined_times$type_pretty <- factor(combined_times$type, labels=c("IQNN   ","KNN-brute   ","AKNN-cover   ", "AKNN-kd   "))
 ggplot()+
  ...
  scale_shape_manual("Neighborhood Algorithm:  ", values=c(17,18,15,19))+
...
n & $k$ & $\delta$ & IQNN & cover & kd & IQNN & KNN & cover & kd \\ 
  ...
  1048576 & 16384 & 8 & 0.423727 & 5.236268 & 0.954038 & 0.001210 & 8.229757 & 0.010522 & 0.010947 \\ 
   \hline
\end{tabular}
\caption{\small Average pre-processing and prediction times from simulations over each parameterization. Horizontal bar separating results from 500 initializations ($n\le 2^{14}$) and 25 initializations ($n\ge 2^{16}$)}
