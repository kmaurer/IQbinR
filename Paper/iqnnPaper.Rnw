...
\hline 
  iris & 150 & archive.ics.uci.edu/ml/machine-learning-databases/iris\\ 
  wpbc & 198 & archive.ics.uci.edu/ml/machine-learning-databases/breast-cancer-wisconsin \\ 
  pima & 768 & archive.ics.uci.edu/ml/machine-learning-databases/pima-indians-diabetes \\
  yeast & 1484 & archive.ics.uci.edu/ml/machine-learning-databases/yeast \\ 
  abalone & 4174 & archive.ics.uci.edu/ml/machine-learning-databases/abalone \\ 
  waveform & 5000 & archive.ics.uci.edu/ml/machine-learning-databases/waveform \\ 
  optdigits & 5620 & sci2s.ugr.es/keel/dataset.php?cod=199 \\ 
  satimage & 6435 & sci2s.ugr.es/keel/dataset.php?cod=71 \\ 
  marketing & 8494 & sci2s.ugr.es/keel/dataset.php?cod=163 \\
  seizure & 11500 & archive.ics.uci.edu/ml/machine-learning-databases/00388 \\ 
\hline 
 & & \\
\textbf{Regression} & & \\ 
\hline 
...
\hline 
  wpbc & 198 & archive.ics.uci.edu/ml/machine-learning-databases/breast-cancer-wisconsin \\ 
  wankara & 321 & sci2s.ugr.es/keel/category.php?cat=reg?cod=41 \\ 
  laser & 993 & sci2s.ugr.es/keel/category.php?cat=reg?cod=47 \\ 
  treasury & 1049 & sci2s.ugr.es/keel/category.php?cat=reg?cod=42 \\
  quake & 2178 & sci2s.ugr.es/keel/category.php?cat=reg?cod=75 \\ 
  skillcraft & 3395 & archive.ics.uci.edu/ml/machine-learning-databases/00272 \\
  anacalt & 4052 & sci2s.ugr.es/keel/category.php?cat=reg?cod=159 \\
  puma & 8192 & sci2s.ugr.es/keel/category.php?cat=reg?cod=1291 \\
  air quality & 9357 & archive.ics.uci.edu/ml/machine-learning-databases/00360 \\
  ccpp & 9568 & archive.ics.uci.edu/ml/machine-learning-databases/00294 \\
   \hline
\end{tabular}
\caption{Sources for data sets used for testing predictive accuracy for models in Section~\ref{accuracy}.}
\label{tab:data_repos}
\end{table}
\normalsize


\section{Appendix B: Model Parameters and Performance}  
\label{accuracyAppend}

The units associated with each feature varible and the number of feature variables differed across all data sets, so data pre-processing was implemented to allow for more uniform comparisons of accuracy for IQNN, KNN and AKNN models. First, we consider only feature variables that are numeric with at least $\sqrt{n}/10$ unique values, thus displaying at least somewhat continuous behavior. Then, a center/scale transformation was used to standardize each feature variable to negate the impact of diverse meaurement units on the distance calculations necessary for the KNN and AKNN approaches. Next, variable selection was performed to narrow the feature space to only two features per data set to simplify comparison across all ten cases. This was done by fitting a random forest to part of the training data removing all but variables with the two highest importance scores; using the average reductions in MSE and Gini index as importance metrics for regression and classification case, respectively. While the variable selection method is somewhat arbitrary here, using high importance variables is done to allow stronger predictions under the constraint than might be had by selecting two features at random. Additionally, we elect to specify the iterative-quantile algorithm to bin the features in order of importance scores. 

\begin{table}[H]
\small
\centering
\begin{tabular}{|l|rcr|rrrr|} \hline
Data & n & tuned-$(\delta_1,\delta_2)$ & tuned-$k$ & IQNN & KNN-brute & AKNN-cover &  AKNN-kd  \\
  \hline
