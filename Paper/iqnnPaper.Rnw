...
An iterative-quantile nearest-neighbor model roughly mimics the neighborhood mechanic of a k-nearest-neighbor model using iterative-quantile bin neighborhoods over the feature space of the training data. To assess the viability of these IQNN models we need to evaluate the computational costs for constructing the binning structure using training observations and the time required and accuracy when making predictions for test observations. We will compare IQNN model performance to KNN, AKNN and instance selection methods using several real data sets. We begin by by investigating the computation times using theoretical and empirical approaches in Subsection~\ref{timing}. We them assess the predictive performance of IQNN models in Subsection~\ref{accuracy}. 
...
%-------------
brute force - KNN & NA & NA & $O(mn(1+log(n)))$ \\
...
To empirically demonstrate the time required for binning and prediction we use simulated training and test data sets, such that we are able to control number of observations and features.
